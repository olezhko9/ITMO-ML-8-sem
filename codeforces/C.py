...
print("%.10f" % (0.0 if sum(weights) == 0 else sum(y_weights) / sum(weights)))


# assert distance.euclidean(query, train[0][:-1]) == euclidean(query, train[0][:-1])
# assert distance.chebyshev(query, train[0][:-1]) == chebyshev(query, train[0][:-1])
# assert distance.cityblock(query, train[0][:-1]) == manhattan(query, train[0][:-1])
